...
$members_connector['members_table'] = "songs_members" ;   
...
function connector_member_pwd($userid,$pwd,$op){
    ...
    if($op=="update"){
        $pwdz = md5($pwd);
    db_query("update songs_members set password='".$pwdz."' where id='".intval($userid)."'",MEMBER_SQL);
    }
}
...
$qr=db_query("select password from songs_members where id='$userid'",MEMBER_SQL);
if(db_num($qr)){
    $data = db_fetch($qr);
...
if(trim($user_email)){
 $qr=db_query("select * from songs_members where email='".db_clean_string($user_email)."'",MEMBER_SQL);
 if(db_num($qr)){
     $data = db_fetch($qr);
 ...
 $data_member = db_qr_fetch("select username,email from songs_members where id='$data[cat]'",MEMBER_SQL);
